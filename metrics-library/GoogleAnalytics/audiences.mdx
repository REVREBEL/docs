...
## Schema Purpose
The `Audiences` table processes performance metrics broken down by designated user audience cohorts.[1] It identifies how specific user segments behave over time, tracking session volumes, engagement, and direct revenue generation.[1] Users are reported in the audiences to which they belonged during the report's date range, meaning current user behavior does not retroactively alter historical audience membership.[1]
