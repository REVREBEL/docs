...
The `Promotions` table measures direct engagement and conversion performance for internal promotions, banners, and marketing campaigns featured on the website or mobile application. This table tracks how effectively creative placements drive item interaction, cart starts, checkout activity, and product revenue.
...
| `itemPromotionClickThroughRate` | FLOAT     | The promotion click-through rate, calculated as promotion clicks divided by promotion views. |
...
Evaluating `itemCheckedOut` alongside `itemsPurchased` highlights potential checkout friction for users who engaged with specific promotional campaigns.
